...
class ScheduleController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        unset($validated['age_groups']);

        $schedule = Schedule::create($validated);

        // Persist age_groups as Postgres TEXT[] (empty = notify all via edge function)
        if (!empty($ageGroups)) {
            ...
            \Illuminate\Support\Facades\DB::statement(
                'UPDATE schedules SET age_groups = NULL WHERE id = ?',
                [$schedule->id]
            );
        }
