...
<div class="space-y-6">
    <div class="flex justify-between items-center border-b border-gray-200 pb-4">
        <div>
            <h1 class="text-xl font-bold text-gray-800">Edit Resident Record</h1>
            <p class="text-sm text-gray-500">Updating information for: <span class="font-semibold text-adminBlue">#{{ str_pad($resident->id, 5, '0', STR_PAD_LEFT) }}</span></p>
        </div>
    ...
    <div class="bg-white border border-gray-200 rounded shadow-sm overflow-hidden">
        <div class="bg-gray-50 px-6 py-3 border-b border-gray-200">
            <h2 class="text-xs font-bold text-gray-600 uppercase">Personal Information</h2>
        </div>

        <form action="{{ route('admin.residents.update', $resident->id) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="last_name" value="{{ old('last_name', $resident->last_name) }}" 
                               class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:ring-1 focus:ring-adminBlue focus:border-adminBlue outline-none" required>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="middle_name" value="{{ old('middle_name', $resident->middle_name) }}" 
                               class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:ring-1 focus:ring-adminBlue focus:border-adminBlue outline-none">
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="number" name="age" id="age" value="{{ old('age', $resident->age) }}" 
                                   class="w-full border border-gray-200 bg-gray-50 rounded px-3 py-2 text-sm font-bold text-gray-500" readonly>
                        </div>
                    </div>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="address" rows="4" class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:ring-1 focus:ring-adminBlue outline-none resize-none" required>{{ old('address', $resident->address) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="mt-8 pt-6 border-t border-gray-100 flex justify-end gap-3">
                ...
                <button type="submit" class="bg-adminBlue hover:bg-blue-600 text-white text-sm font-bold px-8 py-2 rounded shadow-sm transition-all active:scale-95">
                    Save Changes
                </button>
            </div>
        </form>
