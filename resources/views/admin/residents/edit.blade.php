                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="house_no" value="{{ old('house_no', $resident->house_no) }}"
                                   class="w-full bg-slate-50 border-2 border-slate-50 rounded-2xl px-5 py-3 text-sm font-bold text-slate-700 focus:bg-white focus:border-[#1d4ed8] focus:ring-0 outline-none transition-all"
                                   placeholder="e.g. 123">
                        </div>
                        <div>
                            <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">Street</label>
                            <select name="street" class="w-full bg-slate-50 border-2 border-slate-50 rounded-2xl px-5 py-3 text-sm font-bold text-slate-700 focus:bg-white focus:border-[#1d4ed8] focus:ring-0 outline-none transition-all cursor-pointer">
                                <option value="">— Select Street —</option>
                                <option value="Loreto" {{ old('street', $resident->street) === 'Loreto' ? 'selected' : '' }}>Loreto</option>
                                <option value="M. Francisco" {{ old('street', $resident->street) === 'M. Francisco' ? 'selected' : '' }}>M. Francisco</option>
                            </select>
                        </div>
                    </div>
                </div>
